...
// ____________________________________________________________________________________lenis animation on scroll---------------
...
const SideNav = ({
  ...
  // ====== Lenis setup for the specific scrollable sidebar ======
  const scrollRef = useRef(null);    // the overflow-y container
  const contentRef = useRef(null);   // the direct child/content wrapper
  ...
  useEffect(() => {
    ...
    const initLenis = async () => {
      ...
      try {
        // try the canonical package first, fallback to the other common name
    ...
    // run once on mount; do not re-run on every render
  ...
  return (
    <div className="w-full h-[85vh] bg-[#ffffff] p-4 flex flex-col justify-start transition-all duration-300">
      <div>
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          {/* scrollRef is the overflow container */}
          <div
            ref={scrollRef}
            className="overflow-y-auto max-h-[calc(100vh-300px)] custom-scrollbar gap-3 flex flex-col"
          >
            {/* contentRef must be the direct child of wrapper per Lenis docs */}
